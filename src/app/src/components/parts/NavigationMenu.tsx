      ...
      return (
        <li key={index}>
          <Link to={item}>
            {item !== '/user' 
              ? 
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={location.pathname === item ? 2 : 1.5} stroke="currentColor" className="w-8 h-8">
                <path strokeLinecap="round" strokeLinejoin="round" d={Object.values(svgData)[index]} />
              </svg>     
              : 
              <img src={Object.values(svgData)[index]} alt="" className={location.pathname === item ? "w-8 h-8 rounded-full object-cover outline outline-1" : "w-8 h-8 rounded-full object-cover"}/>
            }       
          </Link>
        </li>
      )
